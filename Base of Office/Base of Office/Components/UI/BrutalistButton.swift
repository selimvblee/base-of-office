...
struct BrutalistButton: View {
    ...
    var body: some View {
        ...
        .background(
            Rectangle()
                .fill(Color.black)
                .offset(x: isPressed ? 1 : 2, y: isPressed ? 1 : 2)
        )
        .padding(.trailing, isPressed ? 1 : 2)
        .padding(.bottom, isPressed ? 1 : 2)
...
extension BrutalistButton {
    ...
    static func success(title: String, icon: String? = nil, action: @escaping () -> Void) -> BrutalistButton {
        BrutalistButton(title: title, icon: icon, backgroundColor: AppColors.success, action: action)
    }
}
